...
def _jul_completed_normalize_model_tokens(raw_value):
    ...
    if isinstance(raw_value, (list, tuple, set)):
        ...
        for item in raw_value:
            normalized.extend(_jul_completed_normalize_model_tokens(item))
        return normalized
...
def _jul_completed_entry_source_lots(entry):
    return _jul_ordered_unique(
        _jul_extract_source_lot_id(lot_id)
        for lot_id in (entry.get('combine_lot_ids') or [])
    )


def _jul_completed_int(value, default=0):
    try:
...
def _jul_completed_build_same_model_group_lookup(table_data):
    source_lot_ids = set()
    for entry in table_data:
        source_lot_ids.update(_jul_completed_entry_source_lots(entry))
    ...
    for source_jig in candidate_jigs:
        ...
        for model_no, allocations in by_model.items():
            group_lots = _jul_ordered_unique(row['lot_id'] for row in allocations)
...
def _jul_completed_best_submission(submissions):
    if not submissions:
        return None

    def _score(submission):
        score = _jul_completed_int(getattr(submission, 'total_qty', 0))
...
def _jul_consolidate_same_model_completed_rows(table_data):
    """Collapse only true same-jig/same-model sibling rows for display."""
    if len(table_data) < 2:
        return table_data

    group_lookup = _jul_completed_build_same_model_group_lookup(table_data)
    if not group_lookup:
        return table_data

    all_source_lots = set()
    for entry in table_data:
        all_source_lots.update(_jul_completed_entry_source_lots(entry))
    ...
    for submission in JUSubmittedZ1.objects.filter(
        ...
        source_lot = _jul_extract_source_lot_id(submission.lot_id)
        submissions_by_lot.setdefault(source_lot, []).append(submission)

    grouped_entries = {}
    for entry in table_data:
        source_lots = _jul_completed_entry_source_lots(entry)
        entry_models = set(_jul_completed_normalize_model_tokens(entry.get('no_of_model_cases')))
        if not entry_models:
            entry_models.update(_jul_completed_normalize_model_tokens(entry.get('plating_stk_no')))
        entry_group = None
    ...
    for group_key, entries in grouped_entries.items():
        ...
        for source_lot in _jul_completed_entry_source_lots(entries[0]):
        ...
        def _entry_score(entry):
            score = _jul_completed_int(entry.get('total_case_qty'))
            for source_lot in _jul_completed_entry_source_lots(entry):
                best_submission = _jul_completed_best_submission(submissions_by_lot.get(source_lot, []))
                ...
                if getattr(best_submission, 'tray_data', None):
                    score += 10000
                if _jul_completed_int(getattr(best_submission, 'total_qty', 0)) == _jul_completed_int(entry.get('total_case_qty')):
                    score += 1000
                score += _jul_completed_int(getattr(best_submission, 'total_qty', 0))
            return score

        canonical = dict(sorted(entries, key=_entry_score, reverse=True)[0])
        canonical_source_lots = _jul_completed_entry_source_lots(canonical)
        canonical_submission = None
        for source_lot in canonical_source_lots:
            canonical_submission = _jul_completed_best_submission(submissions_by_lot.get(source_lot, []))
            if canonical_submission and not getattr(canonical_submission, 'is_merged_additional', False):
                break

        canonical_total = _jul_completed_int(canonical.get('total_case_qty'))
        if canonical_submission and not getattr(canonical_submission, 'is_merged_additional', False):
            canonical_total = _jul_completed_int(getattr(canonical_submission, 'total_qty', canonical_total))

        combined_source_lots = _jul_ordered_unique(
            lot_id
            for entry in entries
            for lot_id in _jul_completed_entry_source_lots(entry)
        )
        ...
        tray_capacity = _jul_completed_int(canonical.get('jig_capacity') or canonical.get('tray_capacity'), 1) or 1
        ...
        canonical['all_polishing_stk_nos'] = [group_model_no]
        canonical['unique_polishing_stk_nos'] = [group_model_no]
        ...
        print(
            "[COMPLETED TABLE] Consolidated same-model rows for "
...
class JigUnloading_Completedtable(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        table_data = _jul_consolidate_same_model_completed_rows(table_data)
        print(f"[DEBUG] Total table_data entries after same-model consolidation: {len(table_data)}")
